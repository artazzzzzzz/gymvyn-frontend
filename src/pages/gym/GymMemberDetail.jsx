    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px 4px' }}>
          <span style={{ fontSize: 17, fontWeight: 700, color: '#111' }}>Renew Membership</span>
        ...
        {/* Member info */}
        <div style={{ margin: '8px 20px 16px', padding: '10px 14px', background: '#F7F7F5', borderRadius: 10 }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: '#111', margin: 0 }}>{member?.full_name}</p>
          <p style={{ fontSize: 12, color: '#888', margin: '2px 0 0' }}>Current: {member?.plan_type || '—'}</p>
        </div>
          ...
          {PLAN_OPTIONS.map(plan => (
            <button
              ...
              style={{
                ...
                border: 'none',
                borderLeft: selected === plan.key ? '3px solid #111' : '3px solid transparent',
                borderBottom: '0.5px solid rgba(0,0,0,0.06)',
                textAlign: 'left',
              }}
            >
              <span style={{ fontSize: 15, fontWeight: selected === plan.key ? 600 : 400, color: selected === plan.key ? '#111' : '#444' }}>
                {plan.label}
              </span>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', padding: '0 12px 20px' }}>
          <div style={{
            ...
            <svg width="22" height="22" fill="none" stroke="#A32D2D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button
            onClick={onClose}
            style={{
              flex: 1, height: 50, background: '#F1EFE8', border: 'none',
              borderRadius: 12, fontSize: 15, fontWeight: 600, color: '#555', cursor: 'pointer',
            }}
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            style={{
              flex: 1, height: 50, background: '#A32D2D', border: 'none',
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
        {cells.map((day, i) => {
          ...
          let bg = 'transparent', textColor = '#bbb'
          if (isToday) { bg = '#111'; textColor = '#fff' }
          else if (isFuture) { bg = 'transparent'; textColor = '#ddd' }
          else if (attended) { bg = '#3B6D11'; textColor = '#fff' }
          else { bg = '#F0F0EE'; textColor = '#bbb' }
...
export default function GymMemberDetail() {
  ...
  if (!member) {
    return (
      <div style={{ minHeight: '100vh', background: '#F7F7F5', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
        <p style={{ fontSize: 16, fontWeight: 500, color: '#111' }}>Member not found</p>
        <button onClick={() => navigate(-1)} style={{ fontSize: 13, color: '#185FA5', background: 'none', border: 'none', cursor: 'pointer' }}>← Back</button>
      </div>
    )
  }
  ...
  const daysLeftColor = daysLeft < 7 ? '#A32D2D' : daysLeft < 14 ? '#854F0B' : '#3B6D11'

  const rateVal = member.attendance?.rate_percent ?? 0
  const rateColor = rateVal >= 70 ? '#3B6D11' : rateVal >= 40 ? '#854F0B' : '#A32D2D'
  const rateBg    = rateVal >= 70 ? '#EAF3DE' : rateVal >= 40 ? '#FAEEDA' : '#FCEBEB'

  const churnIsVisible = member.churn_risk && member.churn_risk !== 'low'
  const churnBorderColor = member.churn_risk === 'high' ? '#A32D2D' : '#854F0B'
  const churnBg          = member.churn_risk === 'high' ? '#FCEBEB' : '#FAEEDA'
  const churnTextColor   = member.churn_risk === 'high' ? '#A32D2D' : '#854F0B'
  ...
  return (
    <>
      <div style={{ minHeight: '100vh', background: '#F7F7F5', paddingBottom: 40 }}>
        ...
        <div style={{
          ...
          <span style={{ flex: 1, fontSize: 20, fontWeight: 600, color: '#111' }}>Member Detail</span>
        ...
        <div style={{ padding: '16px 20px 0' }}>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 18, fontWeight: 600, color: '#111', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {member.full_name}
                </p>
                <p style={{ fontSize: 13, color: '#888', margin: '3px 0 6px' }}>
                  Member since {formatMonthYear(member.joined_at || member.created_at)}
                </p>
                {member.plan_type && (
                  <span style={{ background: '#EAF3DE', color: '#3B6D11', fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20 }}>
            ...
            <div style={{
              ...
              background: '#F7F7F5', borderRadius: 10, overflow: 'hidden', marginBottom: 16,
            }}>
              {[
                ...
                <div key={label} style={{
                  ...
                  <p style={{ fontSize: 11, color: '#999', margin: '3px 0 0' }}>{label}</p>
                </div>
              ))}
            </div>
            ...
            {member.phone && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: '0.5px solid rgba(0,0,0,0.06)' }}>
                <svg width="16" height="16" fill="none" stroke="#888" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                ...
                <span style={{ fontSize: 14, color: '#444' }}>{member.phone}</span>
              </div>
            )}
          </div>
          ...
          <div style={card}>
            ...
            <div style={{ height: 6, background: '#F0F0EE', borderRadius: 3, overflow: 'hidden', marginBottom: 6 }}>
              <div style={{ height: '100%', width: `${progressPct}%`, background: '#3B6D11', borderRadius: 3, transition: 'width 0.5s' }} />
            </div>

            {/* Date labels */}
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 11, color: '#aaa' }}>{formatDate(member.membership_start)}</span>
              <span style={{ fontSize: 11, color: '#aaa' }}>{formatDate(member.membership_end)}</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
              <button
                ...
                style={{
                  flex: 1, height: 44, background: '#111', color: '#fff',
              ...
              <button
                ...
                style={{
                  flex: 1, height: 44, background: '#F1EFE8', color: '#111',
                  border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer',
                }}
              >
                Change Plan
            ...
            <div style={{
              ...
              {[
                ...
                <div key={label} style={{
                  ...
                  <p style={{ fontSize: 17, fontWeight: 700, color: extraStyle.color || '#111', margin: 0 }}>{val}</p>
                  <p style={{ fontSize: 11, color: extraStyle.color || '#999', margin: '3px 0 0' }}>{label}</p>
                </div>
              ))}
            </div>
          ...
          {churnIsVisible && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                ...
                {churnScore != null && (
                  <span style={{ fontSize: 12, fontWeight: 600, color: churnTextColor, background: '#fff', padding: '3px 10px', borderRadius: 20 }}>
                    {churnScore} / 100
                  </span>
                )}
              </div>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', fontSize: 13, color: '#185FA5', fontWeight: 600, cursor: 'pointer', padding: 0 }}
              >
                See all →
              </button>
            </div>
            ...
            {!member.payments?.length ? (
              <p style={{ fontSize: 13, color: '#999', textAlign: 'center', padding: '16px 0', margin: 0 }}>No payment records</p>
            ) : (
              member.payments.slice(0, 3).map((payment, i, arr) => (
                <div key={i} style={{
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#EAF3DE', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <svg width="16" height="16" fill="none" stroke="#3B6D11" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                      <circle cx="12" cy="12" r="10" />
                      <path d="M12 6v6l4 2" />
                    </svg>
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 14, fontWeight: 500, color: '#111', margin: 0 }}>Monthly Renewal</p>
          ...
          <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
            ...
            >
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#EBF2FB', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="16" height="16" fill="none" stroke="#185FA5" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                  <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                </svg>
              </div>
              <span style={{ fontSize: 15, fontWeight: 500, color: '#111' }}>Send Message</span>
              <svg style={{ marginLeft: 'auto' }} width="16" height="16" fill="none" stroke="#CCC" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
            ...
            >
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#FCEBEB', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="16" height="16" fill="none" stroke="#A32D2D" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                  <polyline points="3 6 5 6 21 6" />
                  <path d="M19 6l-1 14H6L5 6" />
                  <path d="M10 11v6M14 11v6" />
                  <path d="M9 6V4h6v2" />
              ...
              <span style={{ fontSize: 15, fontWeight: 500, color: '#A32D2D' }}>Remove Member</span>
              <svg style={{ marginLeft: 'auto' }} width="16" height="16" fill="none" stroke="#CCC" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                <polyline points="9 18 15 12 9 6" />
              </svg>
            </button>
