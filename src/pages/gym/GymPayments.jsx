...
function PaymentStatusPill({ status }) {
  const map = {
    ...
    refunded: { bg: '#F1EFE8', color: '#5F5E5A', label: 'Refunded' },
  }
...
function SkeletonRow() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0 16px', height: 60, borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>
      <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#F0F0EE', flexShrink: 0, animation: 'skel 1.2s ease infinite alternate' }} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ height: 13, width: '50%', background: '#F0F0EE', borderRadius: 4, animation: 'skel 1.2s ease infinite alternate' }} />
        <div style={{ height: 11, width: '35%', background: '#F0F0EE', borderRadius: 4, animation: 'skel 1.2s ease 0.2s infinite alternate' }} />
      </div>
      <div style={{ width: 60, height: 13, background: '#F0F0EE', borderRadius: 4, animation: 'skel 1.2s ease infinite alternate' }} />
    </div>
  )
}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {step === 2 && (
              <button onClick={() => setStep(1)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex' }}>
                <svg width="20" height="20" fill="none" stroke="#111" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                  <polyline points="15 18 9 12 15 6" />
                </svg>
              </button>
            )}
            <span style={{ fontSize: 17, fontWeight: 700, color: '#111' }}>Collect Payment</span>
          </div>
          ...
          {step === 1 ? (
              ...
              <div style={{ padding: '0 20px 12px' }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', height: 44, background: '#F1EFE8', border: 'none', borderRadius: 12, padding: '0 12px 0 36px', fontSize: 15, color: '#111', outline: 'none', boxSizing: 'border-box' }}
                  />
                </div>
              </div>
              <div>
                {filteredMembers.slice(0, 20).map((m, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 15, fontWeight: 500, color: '#111', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.full_name}</p>
                        <p style={{ fontSize: 12, color: '#999', margin: '2px 0 0' }}>{m.plan_type || m.membership_type || 'Member'}</p>
                      </div>
                      <svg width="16" height="16" fill="none" stroke="#CCC" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                ...
                {filteredMembers.length === 0 && (
                  <p style={{ textAlign: 'center', fontSize: 13, color: '#aaa', padding: '24px 0' }}>No members found</p>
                )}
              </div>
            </>
          ) : (
            <div style={{ padding: '0 20px' }}>
              ...
              {selectedMember && (() => {
                ...
                return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 14px', background: '#F7F7F5', borderRadius: 10, marginBottom: 16 }}>
                    ...
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 600, color: '#111', margin: 0 }}>{selectedMember.full_name}</p>
                      <p style={{ fontSize: 12, color: '#888', margin: '2px 0 0' }}>{selectedMember.plan_type || 'Member'}</p>
                    </div>
                  </div>
                )
              })()}
                ...
                {PLAN_OPTIONS.map(plan => (
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      borderLeft: selectedPlan === plan.key ? '3px solid #111' : '3px solid transparent',
                      borderBottom: '0.5px solid rgba(0,0,0,0.06)',
                      textAlign: 'left',
                    }}
                  >
                    <span style={{ fontSize: 15, fontWeight: selectedPlan === plan.key ? 600 : 400, color: selectedPlan === plan.key ? '#111' : '#444' }}>
                      {plan.label}
                    </span>
              ...
              <input
                ...
                style={{ width: '100%', height: 40, background: '#fff', border: '0.5px solid rgba(0,0,0,0.12)', borderRadius: 8, padding: '0 12px', fontSize: 13, color: '#111', outline: 'none', boxSizing: 'border-box', marginBottom: 20 }}
              />

              <button
                onClick={handleCollect}
                disabled={submitting}
                style={{ width: '100%', height: 52, background: '#111', color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.7 : 1 }}
              >
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5', paddingBottom: 80 }}>
        ...
        <div style={{
          ...
          <span style={{ flex: 1, fontSize: 20, fontWeight: 600, color: '#111' }}>Payments</span>
          <button style={{ background: 'none', border: 'none', padding: 4, cursor: 'pointer', display: 'flex', minWidth: 44, justifyContent: 'center' }}>
            <svg width="22" height="22" fill="none" stroke="#666" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
          ...
          <button style={{ background: 'none', border: 'none', padding: 4, cursor: 'pointer', display: 'flex', minWidth: 44, justifyContent: 'center' }}>
            <svg width="20" height="20" fill="none" stroke="#666" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
        ...
        <div style={{ padding: '16px 20px 0' }}>
          ...
          {loading ? (
            <div style={{ height: 180, background: '#F0F0EE', borderRadius: 12, marginBottom: 12, animation: 'skel 1.2s ease infinite alternate' }} />
          ) : (
            <div style={card}>
              ...
              <p style={{ fontSize: 28, fontWeight: 700, color: '#111', margin: '0 0 6px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 16 }}>
                {change > 0 ? (
                  <>
                    <svg width="14" height="14" fill="none" stroke="#3B6D11" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                  ...
                  <>
                    <svg width="14" height="14" fill="none" stroke="#A32D2D" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                  ...
                  <span style={{ fontSize: 13, color: '#999' }}>Same as last month</span>
                )}
              </div>

              {/* Stats row */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', background: '#F7F7F5', borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
                {[
                  { val: summary?.transaction_count ?? '—', label: 'Transactions',    color: '#111' },
                  { val: summary?.avg_per_member != null ? `₹${summary.avg_per_member.toLocaleString('en-IN')}` : '—', label: 'Avg. per member', color: '#111' },
                  { val: summary?.overdue_count ?? 0, label: 'Overdue', color: (summary?.overdue_count ?? 0) > 0 ? '#A32D2D' : '#111' },
                  ...
                  <div key={label} style={{ textAlign: 'center', padding: '12px 4px', borderRight: i < 2 ? '0.5px solid rgba(0,0,0,0.08)' : 'none' }}>
                    ...
                    <p style={{ fontSize: 10, color: '#999', margin: '3px 0 0', lineHeight: 1.3 }}>{label}</p>
                  </div>
                ))}
              </div>
                  ...
                  {summary.monthly_chart.map((item, i) => {
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                        <div style={{ width: '100%', height: barH, background: isCurrent ? '#111' : '#3B6D11', borderRadius: '3px 3px 0 0' }} />
          ...
          {!loading && (summary?.overdue_count ?? 0) > 0 && (
            <div style={{
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', fontSize: 12, color: '#A32D2D', fontWeight: 600, cursor: sendingReminders ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap', padding: '4px 0 4px 12px', opacity: sendingReminders ? 0.6 : 1 }}
              >
          ...
          <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
            {loading ? (
              Array.from({ length: 6 }).map((_, i) => <SkeletonRow key={i} />)
            ) : payments.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                <svg width="40" height="40" fill="none" stroke="#ccc" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" style={{ margin: '0 auto 10px', display: 'block' }}>
                  <rect x="2" y="5" width="20" height="14" rx="2" /><line x1="2" y1="10" x2="22" y2="10" />
                </svg>
                <p style={{ fontSize: 14, color: '#999', margin: 0 }}>No payments found</p>
              </div>
            ) : (
          ...
          {!loading && hasMore && (
            <div style={{ textAlign: 'center', paddingBottom: 8 }}>
              <button
                onClick={loadMore}
                style={{ background: 'none', border: 'none', fontSize: 13, color: '#185FA5', fontWeight: 500, cursor: 'pointer', height: 44 }}
              >
                Load {LIMIT} more
              </button>
            </div>
